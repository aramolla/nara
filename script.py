...
ASSET_DIR = os.environ.get("PPS_ASSET_DIR", os.path.join(os.path.dirname(os.path.abspath(__file__)), "model"))
...
RAG_CHARS = 5000   # 프롬프트에 주입하는 법령 조문 블록 전체 글자 예산(항목별 균등 배분)
...
def build_user_prompt(rec: Dict[str, Any], max_chars: int, law_block: str = "") -> str:
    base = (
        f"[공고 ID] {rec['id']}\n\n"
        f"[나라장터 입력 메타]\n{format_meta(rec)}\n\n"
...
def build_messages(rec: Dict[str, Any], system_prompt: str, max_chars: int,
                   law_block: str = "") -> List[Dict[str, str]]:
    return [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": build_user_prompt(rec, max_chars, law_block)},
    ]
...
def fit_to_budget(rec: Dict[str, Any], system_prompt: str, runner, max_chars: int,
                  budget: int = PROMPT_BUDGET, law_block: str = "") -> Tuple[List[Dict[str, str]], int, int]:
    """설정된 토큰 예산에 맞게 문서 글자 수를 조정합니다."""
    while True:
        msgs = build_messages(rec, system_prompt, max_chars, law_block)
...
def run(input_path: str, out_path: str, runner_cls, limit: Optional[int], chunk: int,
    ...
    # 항목별 법령 검색 — 공고의 적용계약법(국가/지방)에 따라 결과가 달라지므로 건별로 계산한다.
    vs = vector_search.VectorSearch(os.path.join(ASSET_DIR, "law_index.faiss"),
                                     os.path.join(ASSET_DIR, "law_chunks.json"))
    log(f"법령 벡터검색 {'준비됨' if vs.ready else '인덱스 없음 → 스킵'}")

    # 전건 메시지 구성(길이 예산 맞춤)
    msgs_all, shrunk, ntok = [], 0, []
    for rec in recs:
        law_block = vector_search.build_law_block(tbl, vs, ITEMS, rec["meta"].get("적용계약법"), RAG_CHARS)
        m, n, mc = fit_to_budget(rec, system_prompt, runner, max_chars, law_block=law_block)
        msgs_all.append(m)
        ntok.append(n)
