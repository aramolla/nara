#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""나라장터 자체입찰 공고 법령 위반사항 모니터링 AI 경진대회 — RAG 베이스라인.

기본 베이스라인(script.py)에 법령 검색을 추가한 실험 버전입니다.
...
전체 흐름
  데이터 로드 → 법령 색인 → 프롬프트 구성(법령 조문 주입) → vLLM 배치 추론 → JSON 파싱
  → 근거 문구 검증 → submission.csv 저장 → 형식 검증

로컬 실행
  python prep/script_rag.py --mock          # 모델 없이 입력·출력 흐름 확인
  python prep/script_rag.py --limit 10      # 앞 10건 실행
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="24개 항목의 법령 위반 여부 판정 RAG 베이스라인")
